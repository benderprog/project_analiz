...
- Mounts `./models` into the container at `/opt/models` (bind mount from bundle folder, no init step).
...
For release bundles, use `compose/.env.docker` generated by `scripts/make_release_bundle.sh` and run compose with `--env-file compose/.env.docker`.

For release bundles, always import images first:

```bash
bash scripts/import_images.sh ./artifacts
```

Then start with `bash scripts/offline_up.sh`. The offline compose override pins `web` to `project_analiz:web-ver-${VERSION}` and disables build mode, so no `docker build` is invoked in closed contour.
