...
SUPPORTED_UPLOAD_EXTENSIONS = {".docx", ".odt", ".pdf"}
SUPPORTED_UPLOAD_ACCEPT = ".docx,.odt,.pdf"
        ...
        for uploaded_file in files:
            ...
            if ext not in SUPPORTED_UPLOAD_EXTENSIONS:
                raise forms.ValidationError(
                    f"Файл {uploaded_file.name} не поддерживается. Допустимые форматы: DOCX, ODT, PDF (с текстовым слоем)."
                )
            try:
